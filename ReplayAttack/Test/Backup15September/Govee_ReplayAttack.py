...
def attack():
    ...
    for f in reversed(list_flows):
        ...
        if f.transport_layer == 'TCP':
            ...
            socket_used.bind(("10.10.0.1",4002))
            socket_used.connect((f.dst_ip, int(f.dst_port)))
        if f.transport_layer == 'UDP':
            socket_used = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)
            socket_used.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            socket_used.bind(('10.10.0.1', 4002))
            mreq = struct.pack("4sl", socket.inet_aton("239.255.255.250"), socket.INADDR_ANY)
            socket_used.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, mreq)
